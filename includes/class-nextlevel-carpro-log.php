...
define('CARPRO_LOG_FOLDER', ABSPATH.'wp-content/carpro-logs');
...
class CARPRO_LOG{
	...
	public static function file(){
		...
		if (!file_exists(CARPRO_LOG_FOLDER)):
		    mkdir(CARPRO_LOG_FOLDER, 0777, true);
		endif;
		...
		$_LOG = trailingslashit(CARPRO_LOG_FOLDER).$_FILE_NAME;

		if(!is_file($_LOG)):
			self::write($_LOG, '-- CARPRO LOG FOR '.$_FILE_DATE.' --');
		endif;
	...
	public static function fetch($_AMT = 10){
		...
		if (file_exists(CARPRO_LOG_FOLDER)):
			...
			$_FILES = scandir(CARPRO_LOG_FOLDER, SCANDIR_SORT_DESCENDING);
	...
	public static function clean(){
		...
		$_FOLDERS_TO_CLEAN = array(CARPRO_LOG_FOLDER);
	...
	public static function link($_LINK, $_RETURN = false){
		...
		$_FILE = trailingslashit(CARPRO_LOG_FOLDER).$_LINK;
